...
class Register(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response( {'id':str(user.id),'refresh': str(refresh),'access': str(refresh.access_token),'message':"Register successfully"})
...
class Post_update(APIView):
    ...
    def put(self,request ,pk): 
        ...
        serializer = User_Post_serializer(instance=blog, data =request.data)
        if serializer.is_valid():
            serializer.save()
        return response.Response({'message': "Your Post update successfully"},status=status.HTTP_205_RESET_CONTENT)
...
class User_Social_Update(APIView):
    ...
    def put(self,request): 
        ...
        if  serializer.is_valid():
            serializer.save()
            return response.Response({'message': "Your Social urls update successfully"},status=status.HTTP_205_RESET_CONTENT)          
        else:
